...
public class ReadExcel {
    ...
    public static List<String[]> readExcel(MultipartFile file,int sheetNum) throws Exception{
        //检查文件
        checkFile(file);
        //获得Workbook工作薄对象
        Workbook workbook = getWorkBookMul(file);
            ...
            //for(int sheetNum = 0;sheetNum < workbook.getNumberOfSheets();sheetNum++){
                ...
                for(int rowNum = firstRowNum+1;rowNum <= lastRowNum;rowNum++){
                    ...
                    for(int cellNum = firstCellNum; cellNum < lastCellNum;cellNum++){
                        ...
                        String cellValue=getCellValue(cell);
                        if(cellValue.equals("")||cellValue.equals("非法字符")||cellValue.equals("未知类型")){
                            throw new Exception(file.getOriginalFilename()+"第"+rowNum+"行,第"+cellNum+"列存在空字符或者非法字符！如果为空数字补零，文字为“空”！");
                        }else{
                            cells[cellNum] = cellValue;
                        }
                    }
    ...
    public static List<String[]> readExcel(MultipartFile file,int sheetNum,int lastCellNum) throws Exception{
        //检查文件
        checkFile(file);
        //获得Workbook工作薄对象
        Workbook workbook = getWorkBookMul(file);
